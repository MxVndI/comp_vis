...
while True:
    ...
    first_moment = cv2.moments(contour)

    square = first_moment['m00']
    if square < 500:
        continue

    object_found = True
    summ_x = first_moment['m10']
    summ_y = first_moment['m01']

    print(f"Контур: Площадь: {square:.0f}, Сумма по x: {summ_x:.0f}, Сумма по y: {summ_y:.0f}")
